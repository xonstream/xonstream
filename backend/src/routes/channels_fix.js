      ...
      */
      ...
      const formattedPosts = (validPosts || []).map(post => {
        ...
        const videoSources = (post.post_video_sources || []).map((vs, index) => ({
          ...
          videoId: vs.video_id,
          embedUrl: `https://streamtape.com/e/${vs.video_id}`,
          downloadUrl: `https://streamtape.com/v/${vs.video_id}`,
          thumbnail: `https://thumb.tapecontent.net/thumb/${vs.video_id}/thumb.jpg`
        }));
